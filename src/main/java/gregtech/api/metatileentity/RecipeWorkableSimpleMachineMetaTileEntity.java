...
/**
 * As legacy {@code SimpleMachineMetaTileEntity}, but built on {@link RecipeWorkableTieredMetaTileEntity}
 * instead of {@code WorkableTieredMetaTileEntity}. A new, parallel class rather than a rewrite of that
 * legacy class itself, for the same reason
 * {@link RecipeWorkableTieredMetaTileEntity} exists alongside {@code WorkableTieredMetaTileEntity}: dozens of
 * registered machines (Electric Furnace, Macerator, Assembler, ...) share that one class, and this migration is
 * opt-in, machine family by machine family &mdash; nothing here changes how any existing machine behaves until its
 * own registration call is switched over to this class.
 * <p>
 * Feature-for-feature port of legacy {@code SimpleMachineMetaTileEntity}: ghost circuit slot, charger slot, auto-output
 * toggles, particles. The only real differences are the handful of spots that touch the recipe-logic
 * trait's API, each using {@code RecipeWorkable}'s equivalent
 * (see {@link #buildUI} and {@link #insufficientEnergy}'s use in place of legacy's {@code isHasNotEnoughEnergy}).
 */
public class RecipeWorkableSimpleMachineMetaTileEntity extends RecipeWorkableTieredMetaTileEntity
    ...
    public RecipeWorkableSimpleMachineMetaTileEntity(ResourceLocation metaTileEntityId, RecipeMap<?> recipeMap,
    ...
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity tileEntity) {
        return new RecipeWorkableSimpleMachineMetaTileEntity(metaTileEntityId, recipeMap, renderer, getTier(),
    ...
    public ModularPanel buildUI(PosGuiData guiData, PanelSyncManager panelSyncManager, UISettings settings) {
        ...
        BooleanSyncValue hasNotEnoughEnergyValue = new BooleanSyncValue(this::insufficientEnergy);

        ModularPanel panel = GTGuis.createPanel(this, 176, 166 + yOffset)
                .child(IKey.lang(getMetaFullName()).asWidget().pos(5, 5))
                .child(recipeMap.getRecipeMapUI()
                        .buildUITemplate(() -> workable.getProgressPercent(0), importItems, exportItems,
